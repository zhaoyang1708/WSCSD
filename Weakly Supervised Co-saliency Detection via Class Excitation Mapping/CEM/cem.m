...
for i = 1:length(imgDataDir)
    ...
    subdir = ['./CEM_50_new/' imgDataDir(i).name];
    mkdir(subdir);
end
...
net_weights = ['../models/resnet/ResCoSal_50class_512_iter_280000.caffemodel'];
net_model = ['../models/resnet/ResCoSal_deploy512.prototxt'];
net = caffe.Net(net_model, net_weights, 'test');
weights_LR1 = net.params('conv50',1).get_data();% get the softmax layer of the network
%     weights_LR[1:2048,1:50] = 0;
weights_LR = reshape(weights_LR1,2048,50); 
...
%                         diff_sum(i,j) = -diff_sum(i,j);%sum->abs
        ...
        TDMname = ['./CEM_50_new/' imgDataDir(k).name '/CEM_' imgDir(im_dir).name];
