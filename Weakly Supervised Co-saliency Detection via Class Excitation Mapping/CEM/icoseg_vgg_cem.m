...
for i = 1:length(imgDataDir)
    ...
    subdir = ['./CEM_icoseg_vgg/' imgDataDir(i).name];
    mkdir(subdir);
end

fid = fopen('/home/vr715/caffe_zy/CAM-master/coseg_vgg_18/labels.txt','r');
labelscell = textscan(fid,'%s');
fclose(fid);
categories = labelscell{1,1};
        
net_weights = ['/home/vr715/caffe_zy/CAM-master/coseg_vgg_18/VGG_icoseg_16_layers__iter_450000.caffemodel'];
net_model = ['/home/vr715/caffe_zy/CAM-master/coseg_vgg_18/icosegVGG16_deplopy.prototxt'];
net = caffe.Net(net_model, net_weights, 'test');
weights_LR1 = net.params('conv50',1).get_data();% get the softmax layer of the network
%     weights_LR[1:2048,1:50] = 0;
weights_LR = reshape(weights_LR1,512,18); 
...
%                         diff_sum(i,j) = -diff_sum(i,j);%sum->abs
        ...
        TDMname = ['./CEM_icoseg_vgg/' imgDataDir(k).name '/CEM_' imgDir(im_dir).name];
